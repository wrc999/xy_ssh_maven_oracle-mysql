...
(function($){
	...
	function setProperties(target){
		...
		function start1(e){
			...
			state.pmask = $('<div class="window-proxy_ssh-mask"></div>').insertAfter(state.window);
			state.pmask.css({
				display: 'none',
				zIndex: $.fn.window.defaults.zIndex++,
				left: e.data.left,
			...
			if (state.proxy_ssh){state.proxy_ssh.remove();}
			state.proxy_ssh = $('<div class="window-proxy_ssh"></div>').insertAfter(state.window);
			state.proxy_ssh.css({
				display: 'none',
				zIndex: $.fn.window.defaults.zIndex++,
				left: e.data.left,
			...
			state.proxy_ssh._outerWidth(e.data.width)._outerHeight(e.data.height);
			state.proxy_ssh.hide();
			setTimeout(function(){
				if (state.pmask){state.pmask.show();}
				if (state.proxy_ssh){state.proxy_ssh.show();}
			}, 500);
		}
		function proc1(e){
			...
			state.proxy_ssh._outerWidth(e.data.width);
			state.proxy_ssh._outerHeight(e.data.height);
		}
