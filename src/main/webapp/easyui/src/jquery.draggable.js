...
(function($){
	function drag(e){
		...
		if (proxy_ssh){
			if (proxy_ssh.parent()[0] == document.body){
	...
	function doDown(e){
		...
		if (!proxy_ssh){
			if (opts.proxy_ssh){
				if (opts.proxy_ssh == 'clone'){
					proxy_ssh = $(e.data.target).clone().insertAfter(e.data.target);
				} else {
					proxy_ssh = opts.proxy_ssh.call(e.data.target, e.data.target);
				}
				state.proxy_ssh = proxy_ssh;
			} else {
				proxy_ssh = $(e.data.target);
			}
		}
		
		proxy_ssh.css('position', 'absolute');
		drag(e);
		applyDrag(e);
	...
	function doUp(e){
		...
		function removeProxy_ssh(){
			if (proxy_ssh){
				proxy_ssh.remove();
			}
			state.proxy_ssh = null;
		}
	...
	$.fn.draggable.methods = {
		...
		proxy_ssh: function(jq){
			return $.data(jq[0], 'draggable').proxy_ssh;
		},
		enable: function(jq){
	...
	$.fn.draggable.defaults = {
		proxy_ssh:null,	// 'clone' or a function that will create the proxy_ssh object, 
